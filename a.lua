--[[
    EGG TRACKER V0.4 (TOGGLE UI + LUCKY EGG BACK) - FIXED SNAPSHOT
    New Features:
    - Toggle button di kiri tengah untuk buka/tutup GUI
    - Lucky Egg Back detection dengan countdown 25 detik
    - Snapshot system diperbaiki - webhook tidak kosong lagi
...
-- Snapshot data saat countdown dimulai (DATA YANG AKAN DIKIRIM KE WEBHOOK)
...
local function scanGarden()
    ...
    if isWaitingForCount and not hasSentWebhook then
        ...
        if remaining > 0 then
            ...
            -- Kirim webhook MENGGUNAKAN DATA SNAPSHOT
            ...
            -- PENTING: Kirim webhook dengan SNAPSHOT data, bukan data current
    ...
    elseif totalReady >= target then
        ...
        if not isWaitingForCount and not hasSentWebhook then
            -- SNAPSHOT: Simpan data egg yang ready SEBELUM countdown dimulai
            snapshotNormalEggs = {}
            snapshotBigEggs = {}
            snapshotTotalReady = totalReady
            
            -- Deep copy normalEggs
            for eggName, pets in pairs(normalEggs) do
